...
class APIservices:
    ...
    def listar_productos(self):
        ...
        # Aplicamos validación a cada producto
        productos_validados = []
        for p in productos:
            try:
                producto_valido = validar_producto(p)
                productos_validados.append(producto_valido)
            except ValueError as e:
                print(f"⚠️ Producto inválido ignorado: {e}")
    ...
    def obtener_producto(self, producto_id):
        ...
        # Validamos el producto antes de retornarlo
    ...
    def crear_producto(self, nuevo_producto):
        ...
        # Si la respuesta tiene contenido, validamos el producto creado
        if response.status_code == 201 and response.text:
            try:
                producto_creado = response.json()
                producto_validado = validar_producto(producto_creado)
                return producto_validado
            except ValueError as e:
                print(f"⚠️ Advertencia: Producto creado pero respuesta inválida: {e}")
...
if __name__ == "__main__":
    ...
    print(f"\n✅ PRODUCTO CREADO:")
    # Si el resultado es un dict (producto validado) o un Response
    if isinstance(resultado, dict):
        print(f"- Producto validado: {resultado}")
    else:
